<template>
    ...
    >
      ...
      <ContactUs />
      <!-- makeDonations -->
      <AllUsers v-if="$store.state.allUsers"/>
<AllPayments v-if="$store.state.allPayments"/>
    </section>
  </template>
...
import AllPayments from "./AllPayments.vue";
  ...
  export default {
    components: { CardThree, VueApexCharts, Sbchart, Profile, ContactUs, AllUsers, AllPayments },
    data() {
      return {
        file: null,
        options: {
